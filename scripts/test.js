...
const EXCLUDED_FILES = ["cnn_face_detector.au3", "face_recognition.au3"];
const INCLUDED_FILES = argv.slice(1);

explore(sysPath.join(cwd, "examples"), (path, stats, next) => {
    const file = sysPath.relative(cwd, path);
    const basename = sysPath.basename(file);
    const extname = sysPath.extname(file);

    if (!INCLUDED_EXT.includes(extname) || EXCLUDED_FILES.includes(basename) || INCLUDED_FILES.length !== 0 && !INCLUDED_FILES.some(include => basename.startsWith(include))) {
        next();
        return;
    }
